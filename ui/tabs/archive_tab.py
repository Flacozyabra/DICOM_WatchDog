...
class ArchiveTab(QWidget):
    ...
    def init_ui(self):
        ...
        if self.main_window:
            ...
            self.table.delete_requested.connect(lambda: self.main_window.delete_archive_patient_action())
    ...
    def populate_table(self, archive_cache=None, config=None, output_field=None):
        if archive_cache is None:
            if self.main_window and hasattr(self.main_window, 'archive_cache'):
                archive_cache = self.main_window.archive_cache
        if archive_cache is None:
            return
        ...
        if selected_ranges:
            ...
            selected_items = getattr(self.main_window, 'selected_archive_items', set())

        if self.main_window:
            self.main_window.selected_archive_items = selected_items

        self.table.setRowCount(0)
        search_text = self.search_entry.text().lower()
        slice_limit = config.get('archive_slice', 0)

        # Filter patients with valid DICOM data and matching search query
        valid_items = {}
        for patient_id, data in archive_cache.items():
            if 'patient_name' not in data or 'study_datetime' not in data or 'folder_datetime' not in data or 'str' not in data:
                if output_field:
                    log_message(output_field, tr_log("log_skipped_archive_patient", patient_id))
                continue
        ...
        if self.main_window and hasattr(self.main_window, 'on_archive_selection_changed'):
            self.main_window.on_archive_selection_changed()
